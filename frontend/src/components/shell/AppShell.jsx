...
export function AppShell({ children }) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#0c0e14] text-slate-900 dark:text-slate-100 overflow-hidden transition-colors">
      <Sidebar isAdmin={isAdmin} getBadge={getBadge} />
      <div className="flex flex-col flex-1 min-w-0">
        <TopHeader />
        <main className="flex-1 overflow-y-auto bg-slate-50 dark:bg-[#0c0e14] transition-colors">{children}</main>
      </div>
    </div>
  )
}
