...
// Replaces AdminWorkflowFleet. Data from /api/v1/admin/workflows/catalog
import { useState, useEffect, useCallback, useMemo } from 'react'
import {
  Layers, Plus, RefreshCw, Search, XCircle, CheckCircle2,
  Edit2, ToggleLeft, ToggleRight, Tag, Plug,
} from 'lucide-react'
...
const CATEGORIES = ['general','productivity','marketing','healthcare','finance','sales','operations']
...
function WorkflowModal({ open, onClose, api, existing, onDone }) {
  ...
  useEffect(() => {
    if (existing) {
      ...
      setForm({ name:'', key:'', description:'', category:'general', status:'active', version:'1.0.0', pricing_model:'included', required_integrations:'', supported_modules:'' })
    }
    setError('')
  }, [existing, open])
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Input label="Workflow Name" required value={form.name}
            ...
            placeholder="Product Launch Sprint" />
          <Input label="Key (slug)" required value={form.key}
            onChange={e => set('key', e.target.value)}
            placeholder="product_launch_sprint"
        ...
        <Input label="Supported Modules (comma-separated)" value={form.supported_modules}
          ...
          placeholder="email, product_launch" />
...
export default function AdminWorkflowCatalog() {
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [catalog, search, filterStatus, filterCategory])
    ...
    <div className="flex flex-col gap-5 p-6 min-h-full bg-slate-50">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {loading ? '…' : `${catalog.length} workflow${catalog.length !== 1 ? 's' : ''} available on platform`}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState icon={Layers} title="No workflows found"
          description={search ? 'Try a different search' : 'Add the first workflow to the platform catalog.'}
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${CATEGORY_COLORS[wf.category] || CATEGORY_COLORS.general}`}>
                      {wf.category}
                    </span>
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${STATUS_COLORS[wf.status] || 'bg-slate-100 text-slate-600'}`}>
                      {wf.status}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => toggleActive(wf)} disabled={toggling === wf.id}
                    ...
                    {wf.active ? <ToggleRight size={16} className="text-emerald-500" /> : <ToggleLeft size={16} />}
                  </button>
                </div>
              </div>
